...
foreach($devices as $key => $device){
	$devs[$key]['devid'] = $device['tabs_ID'];
	$devs[$key]['hotelname'] = $ho->getHotelName($device['hotel_ID']);
	$devs[$key]['roomno'] = $device['room_no'];
	$devs[$key]['maccadd'] = $device['mac_address'];
	$devs[$key]['status'] = $device['status'];
}
...
$data_header = array('title' => DEVICE_TITLE, 'keywords' => MAIN_KEYWORDS, 'description' => MAIN_DESCRIPTION);
...
$data_content = array('devices' => $devs);
...
$content = new Template('views/devicelist.php', $data_content);

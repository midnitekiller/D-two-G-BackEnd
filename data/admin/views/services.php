    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-title-table">
                <label> Services All Items </label>
                <a class="buttonsp" name="action" value="" type="submit" data-toggle="modal" data-target="#services-modal"><i class="fa fa-plus"></i> Create New</a>
            </div>
            <div id="data_handler" class="ibox-content" > 
                <table id="data_display" class="table dynamicDataTables">
                    ...
                    <tbody>
                        <?php foreach ($services as $values => $services_list): ?>
                        <tr class="lists-item">
                            <td class="text-center"><?php echo $services_list['service_ID']; ?></td>
                            <td class="text-center"><img src="media/images/<?=preg_replace("/[^a-zA-Z]+/", "", $services_list['hotel_name']);?>/services/<?=preg_replace("/[^a-zA-Z]+/", "", $services_list['serviceName']);?>/<?=$services_list['image'];?>" class="center-block" width="50" height="50"></td>
                            <td class="text-center"><?php echo $services_list['serviceName']; ?></td>
                            <td class="text-center"><?php echo $services_list['description']; ?></td>
                            <td class="text-center">
                                <a href="update-services.php?id=<?php echo $services_list['service_ID']; ?>" class="fa fa-pencil fa-2x" data-toggle="tooltip" title="Edit" style="color:green;"></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                <a href='javascript:;' name="action" id="<?php echo $services_list['service_ID']; ?>"  onclick='deleteThis(this)' class="fa fa-trash fa-2x" data-toggle="tooltip" title="Delete" style="color:#333;"></a>
                            </td>
                        </tr>
                        <?php endforeach ?>
                    </tbody>
...
<!-- Add Services Button-->
<button class="material-button material-button-toggle" title="Add Services" data-toggle="modal" data-target="#services-modal"><span class="fa fa-plus" aria-hidden="true"></span></button>

<!-- Modal hotel write-up -->
<div id="services-modal" class="modal fade" role="dialog" style="margin-top:-35px;">
                    ...
                    <form method="post" id="create_services_form" enctype="multipart/form-data">
                        <div class="col-sm-12">
                            <div class="col-sm-12"><br/>
                                <label class="control-label">Services Name</label><small> ( required ) </small>
                            ...
                            <div class="form-group">
                                <div class="col-sm-12">
                                    <button class="btn btn-info" name="action" value="Add Services" type="submit" style="float:right">Submit</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
function deleteThis(element) {
    ...
    if(confirm_mssg) {
        ...
        var data = [];
        data.push({"name":"action","value":"Remove Services"});
        data.push({"name":"service_ID","value":service_ID});
        $.post(
            'controller/ServicesController.php',
            data,
            function(info) {
            $('#mssg').html("<div class='alert-success-row'><span class='alert-label alert-label-left alert-success-cell'><i class='glyphicon glyphicon-ok'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Successfully Deleted Services.</p></div>");
            ...
            if(logo_error == 0){ // checking
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if(result == "true") {
                            datatable.destroy();
                            $('#data_handler').load("services.php #data_display", function(){
                            ...
                            $('#service_mssg').html("<div class='alert-success-row'><span class='alert-label alert-label-left alert-success-cell'><i class='glyphicon glyphicon-ok'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Successfully Added Services.</p></div>");
                        }else {
                            $('#service_mssg').html("<div class='alert-danger-row'><span class='alert-label alert-label-left alert-danger-cell'><i class='glyphicon glyphicon-alert'></i></span><p class='alert-body alert-body-right alert-labelled-cell'> Unable to Add Services. Please try again!</p></div>");
                        }
                        $("#loadthis").removeClass('loader-show');

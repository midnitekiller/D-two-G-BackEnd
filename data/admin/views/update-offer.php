<?php 
      ...
      $offer_ID = $_GET['id'];
      $offer_db = new Offer();
      $offer_lists = $offer_db->fetchOfferInformation($offer_ID);
?>
<div id="mssg"></div><br/>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="ibox-content">
                <div class="row">
					<div class="col-sm-3">
                         <img src="media/images/<?=preg_replace("/[^a-zA-Z]+/", "", $offer_list['hotel_name']);?>/offer/<?=preg_replace("/[^a-zA-Z]+/", "", $offer_list['offer_name']);?>/<?=$offer_list['image'];?>" class="center-block"  width="200">
                    </div><br/>
                    <div class="col-sm-9">
						<div class="col-sm-8">
                            <div class="col-sm-6">
						...
						<div class="col-sm-8">
                            ...
                            <div class="col-sm-6"><?php echo $offer_list['description'];?></div>
                        </div>
                        <div class="col-sm-8">
                            <div class="col-sm-6">
...
<div class="col-lg-12">
    <div class="ibox float-e-margins">
        <div class="ibox-title ">
            <h4>Update Offer Details</h4>
        </div>
        <div class="ibox-content">
            <form method="post"  id="edit_offer_form">
            <!--
            |================================
            |=========Services Details=======
            |================================
            -->
                ...
                <div class="row">
                    <input type="hidden" class="form-control" id="offer_ID"  name="offer_ID" value="<?php echo $_GET['id']; ?>"/>
                    <div class="col-sm-4">
                        <label class="control-label">Offer Name</label> <small> ( required ) </small>
                ...
                <div class="row">
                    <div class="form-group">
                        <div class="col-sm-12">
                            <a href="offers.php" class="btn btn-info" name="action" type="submit" style="padding-top:10px;">Back</a>
                            <button class="btn btn-info" name="action" value="Edit Offer" type="submit">Submit</button>
                        </div>
                    </div>
                </div>
            </form>
...
$("#loadthis").addClass('loader-show');
    ...
    var data = [];
    data.push({"name":"action","value":"Get Offer By ID"});
            ...
            if(logo_error == 0){ // checking
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if(result == "true") {
                            $('#mssg').html("<div class='alert-success-row'><span class='alert-label alert-label-left alert-success-cell'><i class='glyphicon glyphicon-ok'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Successfully Updated Offer.</p></div>");
                        }else {
                            $('#mssg').html("<div class='alert-danger-row'><span class='alert-label alert-label-left alert-danger-cell'><i class='glyphicon glyphicon-alert'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Unable to Update Offer. Please try again!</p></div>");
                        }
                        $("#loadthis").removeClass('loader-show');
...
function showFeature(Services) {
    ...
    $("#offer_name").val(service_info[2]);
    $("#descriptions").val(service_info[3]);
    $("#offer_logo").val(service_info[4]);
}
</script>
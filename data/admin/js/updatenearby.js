...
$(function () {
	$("#update_places_detail_form").validate({
		rules: {
			ad_name: "required",
			hotel_name: "required",
			...
			if(imageError == 0){
				$.ajax({
					...
					success: function(data) {
						if(data.trim() == "true") {
							$('#mssg').html("<div class='alert-success-row'><span class='alert-label alert-label-left alert-success-cell'><i class='glyphicon glyphicon-ok'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Successfully Updated Places Nearby Information.</p></div>");
							...
							$('#mssg').html("<div class='alert-danger-row'><span class='alert-label alert-label-left alert-danger-cell'><i class='glyphicon glyphicon-alert'></i></span><p class='alert-body alert-body-right alert-labelled-cell'>Unable to Update Places Nearby. Please try again!</p></div>");
							console.log(data);
						}

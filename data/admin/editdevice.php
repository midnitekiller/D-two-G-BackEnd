...
if(isset($_GET['device_id'])){
	$device = $dev->getDeviceInfo($_GET['device_id']);
	$hotelname = $ho->getHotelName($device['hotel_ID']);
}
...
$data_header = array('title' => EDITDEVICE_TITLE, 'keywords' => MAIN_KEYWORDS, 'description' => MAIN_DESCRIPTION);
$data_nav = array('baseUrl' => BASE_URL, 'name' => $name, 'messages' => $messages);
$data_modal = array('hotels' => $hotels, 'profile' => $profile_lists, 'change' => $change_lists);
$data_content = array('device' => $device, 'hotels' => $hotels, 'hotelname' => $hotelname);
...
$content = new Template('views/editdevice.php', $data_content);
